...
const FoodLoggerApp = () => {
  ...
  // Robust AI call through Netlify function
  const analyzeWithAI = async (prompt, imageData = null) => {
    try {
      // Build messages payload (keep your structured content approach)
      const messages = [{
        role: 'user',
        content: []
      }];
      ...
      console.debug('Calling function URL:', functionUrl);
      ...
      const raw = await response.text(); // read raw once
      console.debug('Function response status:', response.status);
      ...
      if (!response.ok) {
        ...
        try {
          ...
          parsedErr = { message: raw };
        }
        const message = parsedErr.error?.message || parsedErr.error || parsedErr.message || `Function request failed (status ${response.status})`;
        throw new Error(message);
      }

      // Try to parse as JSON object (typical OpenRouter/OpenAI structure)
      let parsed;
      try {
      ...
      // If parsed and it has choices (OpenRouter/OpenAI style), extract the text
      let text = null;
      if (parsed && parsed.choices && parsed.choices.length > 0) {
        const choice = parsed.choices[0];
        // Support both choice.message.content (object/string) and choice.text
        if (choice.message && (typeof choice.message.content === 'string')) {
          text = choice.message.content;
        } else if (choice.message && Array.isArray(choice.message.content)) {
          text = choice.message.content.map(part => {
        ...
        // If we couldn't parse top-level JSON, try to extract JSON object embedded in raw text
        text = raw;
      } else {
        // parsed exists but no choices: fall back to raw
        text = raw;
      }

      // Extract JSON object from returned text (the model is instructed to return JSON)
      const jsonMatch = (typeof text === 'string') ? text.match(/\{[\s\S]*\}/) : null;
      if (jsonMatch) {
        try {
          return JSON.parse(jsonMatch[0]);
        } catch (err) {
          console.warn('Failed to parse JSON from model text match, returning raw text for inspection.', err);
          throw new Error('Could not parse AI response JSON.');
        }
      }

      // If parsed is already an object that looks like the nutrition object, return it
      if (parsed && parsed.nutrition) {
        return parsed.nutrition;
      }

      // As a last attempt, if parsed is an object with direct nutrition fields, return it
      if (parsed && typeof parsed === 'object' && parsed.name && parsed.calories != null) {
        return parsed;
      }

      // Nothing parseable found
      console.error('No JSON nutrition object found in AI response. Raw response:', raw);
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-300">
                    ...
                    Using Google Gemini for food analysis.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="border-2 border-dashed border-gray-600 rounded-lg p-8 text-center cursor-pointer hover:border-gray-500 transition-colors">
                  {isAnalyzing ? (
                    ...
                    <>
                      ...
                      <span className="text-xs text-gray-500 block mt-2">AI will analyze nutritional content</span>
                    </>
                  )}
                </div>
              </label>
